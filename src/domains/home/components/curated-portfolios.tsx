...
interface PortfolioItem {
  ...
  priceRange: string;
  splatter: ElementType;
  splatterHoverClass: string;
  title: string;
}
...
const PORTFOLIO_ITEMS: PortfolioItem[] = [
  {
    ...
    actionHoverColorClass: "hover:text-primary-container",
    actionText: "View Collection (142 Vehicles)",
    badgeColorClass: "bg-primary-container text-on-primary-container",
    badgeText: "Tier I Curation",
    brands: ["Porsche GT", "Aston Martin", "Mercedes-AMG", "BMW M Power"],
    description:
      "Supercars, grand tourers, and executive flagships curated for connoisseurs seeking sheer dynamic capability.",
    icon: MdOutlineSpeed,
    iconColorClass: "text-primary-container",
    priceRange: "$68k - $320k",
    splatter: InkSplatterOne,
    splatterHoverClass: "group-hover:-rotate-3",
    title: "Premium & Performance",
  },
  {
    actionColorClass: "text-tertiary",
    actionHoverColorClass: "hover:text-tertiary-container",
    actionText: "Explore Daily Drivers (340 Vehicles)",
    badgeColorClass: "bg-tertiary text-on-tertiary",
    badgeText: "Tier II Certified",
    brands: ["Audi Q5", "Lexus NX & RX", "Volvo XC60", "Genesis G70"],
    description:
      "Pristine daily crossovers, premium estates, and fuel-efficient family leaders under $45k with meticulous records.",
    icon: MdOutlineEco,
    iconColorClass: "text-tertiary",
    priceRange: "$24k – $45k",
    splatter: InkSplatterTwo,
    splatterHoverClass: "group-hover:rotate-3",
    ...
    <div className="group relative flex flex-col justify-between overflow-hidden rounded-xl border border-surface-variant bg-surface-container-lowest p-space-lg shadow-sm transition-all hover:shadow-md md:p-space-xl">
      ...
      <div className="relative z-10">
        <div className="mb-space-md flex items-center justify-between">
          <span
            className={`rounded-full px-3 py-1 font-label-sm font-semibold text-label-sm uppercase tracking-wider ${badgeColorClass}`}
          >
            {badgeText}
          </span>
          <Icon className={`text-3xl ${iconColorClass}`} />
        </div>
        ...
        <div className="mb-space-xl flex flex-wrap gap-space-xs">
          {brands.map((brand) => (
            <span
              className="rounded-md border border-surface-variant bg-surface-container-low px-2.5 py-1 font-label-sm font-medium text-label-sm text-on-surface-variant"
              key={brand}
            >
              {brand}
            </span>
